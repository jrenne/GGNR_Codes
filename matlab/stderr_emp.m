...
fprintf('Computing finite-difference gradients for standard errors...\n')
for jj = 1:length(par_ind_std)
    j = par_ind_std(jj);
    fprintf('  Parameter %d/%d (index %d)\n', jj, length(par_ind_std), j)
...
stderr_h = nan(size(stderr_g));
stderr_w = nan(size(stderr_g));
CovMat_h_x0 = nan(size(CovMat_g_x0));
CovMat_w_x0 = nan(size(CovMat_g_x0));
J_ij = [];
